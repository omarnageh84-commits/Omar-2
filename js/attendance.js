...
export function renderAttendance(){
  ...
  return `<div style="padding:2px; height:100dvh; display:flex; flex-direction:column; gap:3px">
    <div class="card" style="margin:0; padding:0; overflow:hidden; border-radius:10px; border:1px solid #e5e7eb; flex:0 0 auto">
      ...
      <div style="display:grid; grid-template-columns:1fr 1fr; background:#fff">
        <div style="display:flex; flex-direction:column">
          ...
          <div style="display:flex; justify-content:space-between; align-items:center; padding:0 6px; height:22px; border-bottom:1px solid #f3f4f6; border-left:1px solid #f3f4f6"><span style="font-size:9px; color:#64748b">سعر الساعة علي كم يوم</span><span style="font-size:10px; font-weight:800; color:#10b981">${pricePerHour.toFixed(2)}</span></div>
        ...
        <div style="display:flex; flex-direction:column">
          ...
          <div style="display:flex; justify-content:space-between; align-items:center; padding:0 6px; height:24px; border-bottom:1px solid #f3f4f6"><span style="font-size:9px; color:#64748b">مطلوب عمل يومه</span><span style="font-size:10px; font-weight:700; color:#f59e0b">${dailyNeeded.toFixed(2)}</span></div>
          <div style="display:flex; justify-content:space-between; align-items:center; padding:0 6px; height:24px; border-bottom:1px solid #f3f4f6"><span style="font-size:9px">ديون للصيدليه</span><input data-k="pharmacyDebt" class="inline-edit" type="text" value="${fmtMoney(pharmacyDebt)}" style="width:70px; height:20px; border:1px solid #e5e7eb; border-radius:6px; font-size:10px; text-align:center; font-weight:700"></div>
          <div style="height:22px; border-bottom:1px solid #f3f4f6"></div>
          <div style="display:flex; justify-content:space-between; align-items:center; padding:0 6px; height:24px; background:#fff1f2"><span style="font-size:8px; font-weight:700">المتبقي الصافي ناقص الديون</span><span style="font-size:12px; font-weight:900; color:${remain<0?'#e11d48':'#059669'}">${fmtMoney(remain.toFixed(0))}</span></div>
        </div>
      </div>
    </div>
    <div class="card" style="margin:0; padding:0; overflow:hidden; border-radius:10px; border:1px solid #e5e7eb; flex:1 1 auto; display:flex; flex-direction:column; min-height:0">
      <div style="display:flex; justify-content:space-between; padding:4px 8px; background:#f9fafb; border-bottom:1px solid #e5e7eb; font-size:8px; font-weight:700; flex:0 0 auto"><span>${cur} - ${totalHours.toFixed(1)}س - ${fmtMoney(net.toFixed(0))}ج</span><span style="background:#10b981; color:#fff; padding:1px 8px; border-radius:20px">75% ادخال</span></div>
