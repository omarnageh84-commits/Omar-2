  ...
  <div style="zoom:0.96">
    ...
    <div style="display:flex;flex-direction:column;gap:8px">
      ${sorted.map(n=>{
        ...
        return `<div style="background:${n.color||'#fff'};border:1px solid #e5e7eb;border-radius:14px;padding:10px">
          <div style="display:flex;justify-content:space-between;align-items:start">
            <div style="flex:1">
              ...
              ${n.image? `<img src="${n.image}" onclick="window.open(this.src)" style="width:100%;border-radius:10px;margin-top:6px;max-height:200px;object-fit:cover;cursor:pointer">` : ''}
              ...
              ${total? `<div style="margin-top:6px"><div style="height:4px;background:#e2e8f0;border-radius:99px;overflow:hidden"><div style="height:100%;width:${pct}%;background:#000"></div></div><div style="font-size:8px;color:#64748b">${done}/${total} - ${pct}%</div></div>`:''}
            </div>
...
function deleteNote(id){ if(!confirm('مسح الملاحظة؟')) return; notes=notes.filter(x=>x.id!==id); saveNotes(); renderNotes(); }
...
async function startVoiceNote(){
  ...
  try{
    ...
    mediaRecorder.onstop=()=>{ let blob=new Blob(audioChunks,{type:'audio/webm'}); let reader=new FileReader(); reader.onload=e=>{ tempAudio=e.target.result; document.getElementById('attachPreview').innerHTML+='🎤 تسجيل مرفق<br>'; stream.getTracks().forEach(t=>t.stop()); }; reader.readAsDataURL(blob); };
    mediaRecorder.start(); btn.innerText='⏹️'; btn.style.background='#fee2e2';
  }catch(e){ alert('لازم تسمح للميكروفون من إعدادات كروم - المتصفح قافل الصوت'); }
}
...
function initDraw(){
  ...
  c.onmousemove=e=>{ if(!drawing) return; let p=getPos(e); ctx.lineWidth=2; ctx.lineCap='round'; ctx.lineTo(p.x,p.y); ctx.stroke(); ctx.beginPath(); ctx.moveTo(p.x,p.y); };
