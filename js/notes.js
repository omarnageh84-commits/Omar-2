...
function renderNotes(){
  ...
  <div style="zoom:0.96">
    <div id="noteComposer" style="background:#fff;border:2px solid #000;border-radius:16px;padding:10px;margin-bottom:10px;box-shadow:4px 4px 0 #000">
      ...
      <div id="attachPreview" style="margin-top:6px;font-size:8px"></div>
    </div>
    <div style="display:flex;flex-direction:column;gap:8px">
        ...
        <div style="background:${n.color==='#fef08a'?'#fefce8':n.color==='#bbf7d0'?'#f0fdf4':n.color==='#bfdbfe'?'#eff6ff':n.color==='#fec8f8'?'#fdf2f8':'#f5f3ff'};border:1px solid #e5e7eb;border-radius:14px;padding:10px;position:relative">
          <div style="display:flex;justify-content:space-between;align-items:start">
            <div style="flex:1">
              ...
              ${total? `<div style="margin-top:6px"><div style="height:4px;background:#e2e8f0;border-radius:99px;overflow:hidden"><div style="height:100%;width:${pct}%;background:#000;transition:.3s"></div></div><div style="font-size:8px;margin-top:2px;color:#64748b">${done}/${total} - ${pct}%</div></div>`:''}
          ...
          <div style="margin-top:6px;display:flex;gap:4px">
            <input id="subIn${n.id}" placeholder="+ فرعية سريعة" style="flex:1;font-size:9px;padding:4px 8px;border-radius:99px;border:1px solid #e2e8f0;background:#fff;color:#000">
      ...
      }).join('') || '<div style="text-align:center;padding:20px;color:#94a3b8;font-size:11px">لا يوجد ملاحظات</div>'}
    </div>
  </div>`;
  initDraw();
}
...
function renderTempSubs(){
  let c=document.getElementById('tempSubs'); if(!c) return;
  c.innerHTML=tempSubs.map((s,i)=>`<div style="display:flex;gap:4px;align-items:center;background:#f1f5f9;padding:4px 8px;border-radius:8px;font-size:10px"><span>• ${s.text}</span><b onclick="tempSubs.splice(${i},1);renderTempSubs()" style="color:#ef4444;cursor:pointer;margin-right:auto">✕</b></div>`).join('');
}
let tempImage=null,tempAudio=null,tempDraw=null;
function attachImage(input){
  let file=input.files[0]; if(!file) return;
  let reader=new FileReader(); reader.onload=e=>{ tempImage=e.target.result; document.getElementById('attachPreview').innerHTML+=`<div>🖼️ صورة مرفقة</div>`; }; reader.readAsDataURL(file);
}
...
function addNote(){
  ...
  let canvas=document.getElementById('drawCanvas'); if(canvas &&!canvas.classList.contains('hidden')) tempDraw=canvas.toDataURL();
  if(editingNoteId){
...
function deleteNote(id){
  ...
  if(!confirm(`مسح الملاحظة "${n.title||''}" ؟`)) return;
...
function toggleSub(id,i){ let n=notes.find(x=>x.id===id); n.subs[i].done=!n.subs[i].done; saveNotes(); renderNotes(); }
function delSub(id,i){ if(!confirm('مسح الفرعية؟')) return; let n=notes.find(x=>x.id===id); n.subs.splice(i,1); saveNotes(); renderNotes(); }
async function startVoiceNote(){
  ...
  try{
    ...
    mediaRecorder.onstop=()=>{ let blob=new Blob(audioChunks,{type:'audio/webm'}); let reader=new FileReader(); reader.onload=e=>{ tempAudio=e.target.result; document.getElementById('attachPreview').innerHTML+=`<div>🎤 تسجيل مرفق</div>`; }; reader.readAsDataURL(blob); };
    mediaRecorder.start(); btn.innerText='⏹️'; btn.style.background='#fee2e2';
  }catch(e){ alert('الميك مش شغال'); }
}
