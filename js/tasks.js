...
export function renderTasks(){
  ...
  if(openId){
    ...
    if(!t){ openId=null; } else {
      if(!t.subtasks) t.subtasks=[]; if(!t.attachments) t.attachments=[];
      ...
      <div style="display:flex; justify-content:space-between; padding:10px; background:#0f172a; color:#fff; position:sticky; top:0; z-index:10">
        ...
        <span style="font-size:10px">${t.subtasks.filter(s=>s.done).length}/${t.subtasks.length} فرعية</span>
      </div>
      <div style="padding:12px">
        ...
        <div style="background:#f8fafc; border:1px solid #e2e8f0; border-radius:12px; padding:10px;">
          ...
          <div style="display:flex; gap:4px; margin-top:8px"><input id="newSubTask" placeholder="مهمة فرعية..." style="flex:1; padding:8px; border:1px solid #e2e8f0; border-radius:8px; font-size:11px"><button class="btn-sm btn-dark" data-action="addSub" data-id="${t.id}">+</button></div>
          <div style="margin-top:8px">${t.subtasks.map(s=>`<div style="display:flex; align-items:center; gap:6px; background:#fff; border:1px solid #f1f5f9; padding:6px 8px; border-radius:8px; margin-top:4px"><input type="checkbox" ${s.done?'checked':''} data-action="toggleSub" data-id="${t.id}" data-subid="${s.id}"><span style="flex:1; font-size:11px; ${s.done?'text-decoration:line-through; color:#94a3b8':''}">${s.text}</span><button class="btn-sm" style="background:#fee2e2; color:#e11d48; padding:2px 6px; border-radius:6px; font-size:10px" data-action="delSub" data-id="${t.id}" data-subid="${s.id}">✕</button></div>`).join('') || '<div style="font-size:9px; color:#94a3b8; text-align:center; padding:6px">لا مهام فرعية</div>'}</div>
        </div>
        <div style="border:1.5px solid #e2e8f0; border-radius:12px; overflow:hidden; margin-top:12px">
          ...
          <div style="display:grid; grid-template-columns:1fr 1fr 1fr; gap:6px; padding:8px">
            ...
            <button class="btn-sm" style="background:${isRecording?'#fee2e2':'#fef3c7'}; border:1px solid #fde68a; padding:10px 4px; border-radius:10px; font-size:10px; font-weight:700" data-action="toggleRec" data-id="${t.id}">${isRecording?'⏹️ إيقاف':'🎙️ فويس'}</button>
          </div>
          <input type="file" id="taskImageInput" accept="image/*" style="display:none"><input type="file" id="taskCameraInput" accept="image/*" capture="environment" style="display:none">
        ...
        <div style="border:1.5px solid #e2e8f0; border-radius:12px; overflow:hidden; margin-top:10px"><div style="display:flex; justify-content:space-between; padding:8px; background:#f8fafc"><b style="font-size:10px">🎨 رسم</b><div style="display:flex; gap:4px"><button class="btn-sm" style="background:#fff; border:1px solid #e2e8f0" data-action="clearDraw">مسح</button><button class="btn-sm btn-dark" data-action="saveDraw" data-id="${t.id}">حفظ</button></div></div><canvas id="drawCanvas" width="340" height="200" style="width:100%; background:#fff; touch-action:none"></canvas></div>
...
export function handleTasks(btn,e,rerender){
  let tasks=L(KEY,[]);
  if(e.target.id==='taskImageInput' && e.target.files[0]){ const file=e.target.files[0]; const reader=new FileReader(); reader.onload=()=>{ const t=tasks.find(x=>x.id===openId); if(t){ if(!t.attachments) t.attachments=[]; t.attachments.push({type:'image', data:reader.result}); S(KEY,tasks); rerender(); setTimeout(()=>{ const cv=document.getElementById('drawCanvas'); if(cv) initDraw(cv,t); },100); } }; reader.readAsDataURL(file); return; }
  if(e.target.id==='taskCameraInput' && e.target.files[0]){ const file=e.target.files[0]; const reader=new FileReader(); reader.onload=()=>{ const t=tasks.find(x=>x.id===openId); if(t){ if(!t.attachments) t.attachments=[]; t.attachments.push({type:'image', data:reader.result}); S(KEY,tasks); rerender(); setTimeout(()=>{ const cv=document.getElementById('drawCanvas'); if(cv) initDraw(cv,t); },100); } }; reader.readAsDataURL(file); return; }
  let cats=getCats();
  ...
  if(btn.dataset.action==='toggleRec'){
    ...
    if(!isRecording){
      navigator.mediaDevices.getUserMedia({audio:true}).then(stream=>{
        ...
        mediaRecorder.onstop=()=>{ const blob=new Blob(audioChunks,{type:'audio/webm'}); const reader=new FileReader(); reader.onload=()=>{ if(t){ if(!t.attachments) t.attachments=[]; t.attachments.push({type:'audio', data:reader.result}); S(KEY,tasks); isRecording=false; rerender(); setTimeout(()=>{ const cv=document.getElementById('drawCanvas'); if(cv) initDraw(cv,t); },100); } }; reader.readAsDataURL(blob); };
